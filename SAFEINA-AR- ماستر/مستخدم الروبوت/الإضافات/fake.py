...
from safeina1.razan.resources.strings import *
...
@safeina1.on(admin_cmd(pattern="كتابة(?: |$)(.*)"))
async def _(event):
...
@safeina1.on(admin_cmd(pattern="صوتية(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@safeina1.on(admin_cmd(pattern="فيد(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@safeina1.on(admin_cmd(pattern="لعبة(?: |$)(.*)"))
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@safeina1.on(admin_cmd(pattern="الرابط$"))
async def _(e):
...
@safeina1.on(admin_cmd(pattern="للكل تاك$"))
async def listall(safeina1):
    if safeina1.fwd_from:
        return
    mentions = "- هذه هي قائمة جميع الاعضاء هنا: "
    chat = await bot.get_input_chat()
    async for x in borg.iter_participants(chat, 2000):
        mentions += f" \n[{x.first_name}](tg://user?id={x.id})"
    await safeina1.reply(mentions)
    await safeina1.delete()
...
@safeina1.on(admin_cmd(pattern=r"سبونج"))
async def kerz(kerz):
    await kerz.edit(R)
...
@safeina1.on(admin_cmd(pattern=r"كلب"))
async def dog(dog):
    await dog.edit(M)
Z = (
    "┈┏━╮╭━┓┈╭━━━━╮\n"
...
@safeina1.on(admin_cmd(pattern=r"ذئب"))
async def fox(fox):
    await fox.edit(H)


@safeina1.on(admin_cmd(pattern=r"فيل"))
async def elephant(elephant):
    await elephant.edit(A)


@safeina1.on(admin_cmd(pattern=r"هومر"))
async def homer(homer):
    await homer.edit(N)


@safeina1.on(admin_cmd(pattern=r"بك"))
async def pig(pig):
    await pig.edit(Z)
