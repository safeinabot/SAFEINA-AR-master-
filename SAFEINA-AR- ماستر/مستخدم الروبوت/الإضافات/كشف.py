...
async def fetch_info(replied_user, event):
    ...
    rotbat = ".「  مطـور السورس 𓄂𓆃 」." if user_id ==  1924763504 else (".「  العضـو 𓅫  」.") 
    rotbat = ".「 مـالك الحساب 𓀫 」." if user_id == (await event.client.get_me()).id and user_id !=  5193718748 else rotbat
...
@Safeina.on(admin_cmd(pattern=f"{ID_EDIT}(?:\s|$)([\s\S]*)"))
async def who(event):
