...
#edit  ~ @S_F_M_L for safeina 
#جميع الحقوق محفوظة لسـورس سـفينه تخـمط تبيـن فشلـك
...
async def tmgif(event):
    if event.fwd_from:
        return
    S_F_M_L = await reply_id(event)
    if sad:
        safeina = f"**˛ JEP ، ٰ𝖦𝗂𝖿 𝖲ِ𝖺ٰ𝖣 . .**\n"
        safeina += f"✛━━━━━━━━━━━━━✛\n"
        safeina += f"**الـمتحـرڪـة الأولـى **"
        await event.client.send_file(event.chat_id, sad, caption=safeina, reply_to=S_F_M_L)
