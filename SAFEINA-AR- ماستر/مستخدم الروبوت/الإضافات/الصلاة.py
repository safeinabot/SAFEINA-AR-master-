#ترجمه فريق  سہٰفہٰيٰنہٰهٰہٰٖ  على التيلكرام
import json
...
async def get_safeina(safeina):
    LOKASI = safeina.pattern_match.group(1)
    url = f"https://api.pray.zone/v2/times/today.json?city={LOKASI}"
    request = requests.get(url)
    if request.status_code != 200:
        await edit_delete(
            safeina, f"** لم يـتم العثور على معلومات لـهذه المدينه {LOKASI}**\n يرجى كتابة اسم محافظتك وباللغه الانكليزي ", 5
        ) #ترجمه فريق  سہٰفہٰيٰنہٰهٰہٰٖ  على التيلكرام
        return
    result = json.loads(request.text)
    onresult = f"<b>اوقـات صـلاه المـسلمين 👳‍♂️ </b>\
    ...
    "
    await edit_or_reply(safeina, safeina result, "html")

# Copyright (C)2022 Safeina
# FILES WRITTEN BY  @S_F_M_L