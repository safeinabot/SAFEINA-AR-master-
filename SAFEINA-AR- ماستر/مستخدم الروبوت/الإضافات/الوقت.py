...
from pytz import country_safeinas as c_tz
from pytz import safeina as tz
...
async def get_tz(con):
    """Get time safeina of the given country."""
    if "(Uk)" in con:
...
@safeina.ar_cmd(
    ...
    info={
        ...
        "usage": "{tr}ctime <country name/code> <safeina number>",
...
async def time_func(tdata):
    ...
    if len(con) > 4:
        try:
            ...
            c_name = con
        safeinas = await get_tz(con)
    elif Config.COUNTRY:
        c_name = Config.COUNTRY
        tz_num = Config.TZ_NUMBER
        safeinas = await get_tz(Config.COUNTRY)
    else:
    ...
    if len(safeinas) == 1:
        time_safeina = safeinas[0]
    elif len(safeinas) > 1:
        if tz_num:
            tz_num = int(tz_num)
            time_safeina = safeinas[tz_num - 1]
        else:
            return_str = f"`{c_name} has multiple safeinas:`\n\n"

            for i, item in enumerate(safeinas):
    ...
    dtnow1 = dt.now(tz(time_safeina)).strftime(t_form)
    dtnow2 = dt.now(tz(time_safeina)).strftime(d_form)
    if c_name != Config.COUNTRY:
        await edit_or_reply(
            tdata,
            f"`It's`  **{dtnow1}**` on `**{dtnow2}**  `in {c_name} ({time_safeina} safeina).`",
        )
    if Config.COUNTRY:
        await edit_or_reply(
            tdata,
            f"`It's`  **{dtnow1}**` on `**{dtnow2}**  `here, in {Config.COUNTRY}"
            f"({time_safeina} safeina).`",
        )
