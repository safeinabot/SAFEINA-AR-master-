...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@safeina.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(userbot):
    ...
    try:
        ...
        pass
    if me == user:
       safeina = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
...
@safeina.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(userbot):
    dc = razan = userbot
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       safeina = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
