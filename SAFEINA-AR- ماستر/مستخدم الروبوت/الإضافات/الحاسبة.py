...
@Safeina .on(admin_cmd(pattern="حاسبة(?:\s|$)([\s\S]*)"))
async def icalc(e):
    if e.client._bot:
        return await e.reply(
            "**الحـاسبة العـلمية لسـورس سفينه\n @safeina1**", buttons=lst
        )
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**الحـاسبة العـلمية لسـورس سفينه\n @safeina1**", buttons=lst
        )
        await event.answer([calc])


# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @safeina1 ~ @S_F_M_L
@Safeina .tgbot.on(CallbackQuery(data=re.compile(b"calc(.*)")))
@check_owner
...
# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @safeina1 ~ @S_F_M_L
@Safeina .tgbot.on(CallbackQuery(data=re.compile(b"recalc")))
@check_owner
async def _(e):
    m = [
        "AC",
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**الحـاسبة العـلمية لسـورس سفينه\n @safeina1**", buttons=lst)
