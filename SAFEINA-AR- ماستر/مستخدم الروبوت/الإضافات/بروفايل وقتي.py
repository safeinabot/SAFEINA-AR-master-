...
from . import AUTONAME, DEFAULT_BIO, edit_delete, safeina1, logging
...
digitalpfp = Config.DIGITAL_PIC or "https://telegra.ph/file/d46ae74ee596000f78715.jpg"
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await safeina1.upload_file(autophoto_path)
        try:
            if i > 0:
                await safeina1(
                    functions.photos.DeletePhotosRequest(
                        await safeina1.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await safeina1(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await safeina1(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await safeina1(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
@safeina1.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@safeina1.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@safeina1.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
safeina1.loop.create_task(digitalpicloop())
safeina1.loop.create_task(autoname_loop())
safeina1.loop.create_task(autobio_loop())
