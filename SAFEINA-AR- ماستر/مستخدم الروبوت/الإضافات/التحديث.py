...
from userbot import HEROKU_APP, UPSTREAM_REPO_URL, safeina, CMD_HELP
...
async def print_changelogs(event, ac_br, changelog):
    changelog_str = (
        f"**⌯︙قام مطورين السورس بتحديث سـفينـه**\n⌯︙**التـغييرات\n** {changelog}"
    )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    jasme = await event.edit(
        "** ⌯︙تم تحديث سورس سـفينـه بنجاح انتظر قليلا سوف نخبرك بعد اعادة التشغيل !**"
    )
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "**⌯︙سورس سـفينـه محدث الى اخر اصدار **\n"
            f"**قـنـاة سـورس جيـبثون** : @safeina1"
        )
        return repo.__del__()
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"⌔ :  لتحديث سورس سـفينـه ارسل : `.تحديث الان` "
        )
