...
async def ban_user(chat_id, i, rights):
    try:
        await safeina(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
@safeina.ar_cmd(
    ...
    info={
        "header": "⌔︙لإنشاء مجموعة خاصة/قناة مع سورس سفينه  ☸️",
...
async def _(event):
    ...
    if type_of_group == "قناه":
        descript = "⌔︙ هذه قناة إختبار أُنشئت بإستعمال سورس سفينه"
    else:
        descript = "⌔︙ هذه المجموعه إختبار أُنشئت بإستعمال سورس سفينه"
...
async def _(event):  # sourcery 𝑺𝑨𝑭𝑬𝑰𝑵𝑨 𖠢
