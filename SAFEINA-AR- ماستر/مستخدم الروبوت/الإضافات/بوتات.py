...
@safeina.on(admin_cmd(pattern="حالتي ?(.*)"))
async def _(event):
...
@safeina.on(admin_cmd(pattern="الاغنية ?(.*)"))
async def _(event):
...
@safeina.on(admin_cmd(pattern="ايميل وهمي(?: |$)(.*)"))
async def _(event):
    chat = "@TempMailBot"
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            response = await response
            safeina = (response).reply_markup.rows[2].buttons[0].url
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
