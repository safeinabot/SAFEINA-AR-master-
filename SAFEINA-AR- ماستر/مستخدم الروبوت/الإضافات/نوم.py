...
@safeina.ar_cmd(outgoing=True, edited=False)
...
async def on_afk(event):  # sourcery no-metrics
    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        ...
        if AFK_.afk_type == "media":
            if AFK_.reason:
                message_to_reply = (
                    f"- **حبيبي يغالي انا دلوقتي نايم هرد عليك اول مافتح** \n- **مـدة الـنوم :** {endtime}"
                )
            else:
                message_to_reply = f"- **حبيبي يغالي انا دلوقتي نايم هرد عليك اول مافتح** \n- **مـدة الـنوم :** {endtime}"
            if event.chat_id:
